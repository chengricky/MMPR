...
public:
	...
	void computeResized( const cv::Mat& _image, cv::Mat& _descriptors) const;
	// Compute the LDB for a resized image by employing several keypoints distributed on a grid.
	void computeResizedMultipoint( const cv::Mat& _image, cv::Mat& _descriptor, int _gridx, int _gridy) const;
