...
static image_list_t *create_gabor(int nscales, const int* _or,int width, int height)
{
    int i, j, fn;
    ...
    int nfilters = 0;
    for(i=0;i<nscales;i++)  nfilters+= _or[i];
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= _or[i-1]; j++)
        {
            ...
            param[l][2] = 16*pow(_or[i-1], 2)/pow(32, 2);
            param[l][3] = M_PI/(_or[i-1])*(j-1);
            l++;
        }
    }
...
float *bw_gist_scaletab(image_t *src, int w, int n_scale, const int *n_orientation, bool isNormalize)
{
    int i;
...
float *color_gist_scaletab(color_image_t *src, int w, int n_scale, const int *n_orientation, bool isNormalize)
{
    int i;
